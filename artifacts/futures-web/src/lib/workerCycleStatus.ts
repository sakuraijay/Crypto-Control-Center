 ...
 * 대신 lastCycleAt 최근성(5분)과 cycleCount로 판정한다.
 */

export const WORKER_ACTIVE_WINDOW_MS = 5 * 60 * 1000;
/** 허용 시계 오차 — 이보다 미래인 lastCycleAt은 신뢰 불가(unknown) */
...
export function classifyWorkerCycleStatus(input: {
  ...
  const { lastCycleAt, cycleCount } = input;
  if (lastCycleAt === undefined || lastCycleAt === null || lastCycleAt === '') return 'unknown';
  const t = Date.parse(lastCycleAt);
