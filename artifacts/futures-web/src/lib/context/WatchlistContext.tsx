...
export function WatchlistProvider({ children }: { children: ReactNode }) {
  const [watchlist, setWatchlist] = useState<WatchlistSymbol[]>(() => {
    try {
      ...
      return saved ? (JSON.parse(saved) as WatchlistSymbol[]) : MOCK_WATCHLIST;
    } catch {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setWatchlist(prev => prev.map(item => {
        ...
        return { ...item, price: item.price + pNoise, score1h, score4h, score1d, combinedScore };
      }));
    }, 3000);
  ...
  const addSymbol = useCallback((sym: string) => {
    ...
    setWatchlist(prev => {
      ...
      return [...prev, {
        ...
        volume24h: 1_000_000 + Math.random() * 10_000_000,
