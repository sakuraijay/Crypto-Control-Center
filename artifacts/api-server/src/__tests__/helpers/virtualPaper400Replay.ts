...
export function virtualReplaySignal(nowMs: number): StrategyShadowRecord {
  ...
  const record: StrategyShadowRecord = {
    ...
    mode: 'SHADOW_ONLY', symbol: 'BTC', evaluatedAt: nowMs, sourceCandleCloseTime: close,
    regime: 'TREND_UP', action: 'LONG', comparison: 'ENSEMBLE_ONLY', strategyId: 'TREND_PULLBACK',
    signalId: `REPLAY:BTC:${close}`, direction: 'LONG', confidence: 85, selectedScore: 85,
  ...
  record.candleSignalEvidence = buildCandleStrategyShadowEvidence({
    candleSignal: { schemaVersion: 'candle-signal/v1', symbol: 'BTC', evaluatedAtMs: nowMs,
    ...
    v2Regime: { configVersion: 'regime-engine/v2', symbol: 'BTC', calculatedAt: close } as never,
    shadowRecord: record,
  })!;
  return record;
}
