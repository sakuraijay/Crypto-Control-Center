...
function identity(workspaceTree = TREE): ReleaseIdentity {
  return {
    schemaVersion: 1,
    releaseSha: SHA,
    productTree: TREE,
...
describe('operational configured/effective diagnostics', () => {
  ...
  it('explains enabled configuration that is ineffective at runtime', () => {
    const result = deriveOperationalDiagnostics({
    ...
    expect(result.flags.gmxOrderSubmissionEnabled.driftReason).toMatch(/signer/);
  });
