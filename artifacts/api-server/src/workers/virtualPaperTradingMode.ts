...
export interface VirtualTradePlan {
  version: typeof MODE_VERSION; mode: VirtualTradingMode; basis: 'INITIAL_POSITION_MARGIN_NET_ESTIMATED';
...
export function parseVirtualTradePlan(value: unknown): VirtualTradePlan | null {
  ...
  const rebuilt = buildVirtualTradePlan({ mode: p.mode, entryPrice: p.entryPrice, structuralStop: p.structuralStop,
...
export function tradingModeExit(row: DbTrade, rawPlan: unknown, price: number, nowMs: number): string | null {
  ...
  if (roe >= plan.targetRoePct) return 'MODE_NET_TAKE_PROFIT';
  return null;
}
