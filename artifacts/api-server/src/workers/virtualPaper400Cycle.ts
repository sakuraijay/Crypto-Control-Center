...
export async function runVirtualPaper400Cycle(d: VirtualPaper400CycleDeps) {
  ...
  const outcome = (status: string, reason: string | null = null) => ({
    status, reason, at: d.now.toISOString(), mode: 'VIRTUAL_PAPER_400' as const,
  ...
  if (account.lastOpenAtMs !== null && d.now.getTime() - account.lastOpenAtMs < 30 * 60_000) {
    return outcome('NO_TRADE', 'VIRTUAL_COOLDOWN');
  }
  const capital = Math.max(0, Math.min(400, account.equityUsd ?? 0));
  const profile: AppliedRiskProfileSnapshot = { name: 'conservative', version: 'risk-profile/v1',
  ...
  for (const signal of signals) {
    ...
    if (decision.action === 'REJECT' || !signal.signalId || signal.confidence === null
      ...
      || entryNow.getTime() - signal.sourceCandleCloseTime > 15 * 60_000) continue;
    ...
    if (!q || !market || !Number.isFinite(q.priceUsd) || q.priceUsd <= 0 || !Number.isFinite(q.ageMs)
      || q.ageMs < 0 || q.ageMs > 60_000 || signal.structuralStop === null || signal.entryPrice === null) continue;
    ...
    if (stopDistance <= 0 || stopDistance >= 0.5 || Math.abs(q.priceUsd / signal.entryPrice - 1) > 0.005) continue;
    const requested = Math.min(profile.derivedLimits.maxMarginPerTradeUsd,
    ...
    if (!cost || cost.source !== 'PAPER_GMX_ESTIMATE') continue;
    ...
    if (!freshQuote || freshQuote.priceUsd !== q.priceUsd || !Number.isFinite(freshQuote.ageMs)
      || freshQuote.ageMs < 0 || freshQuote.ageMs > 60_000) continue;
    ...
    if (!checked.ok || checked.effectiveRoundTripCostUsd > 0.40) continue;
    const sizing = enforceOrderSizing({ requestedSizeUsd: requested, requestedCollateralUsd: requested,
      requestedLeverage: 1, positionSizingCapitalUsd: capital, stopDistanceFraction: stopDistance,
      costSnapshot: cost, liquidityCapUsd: profile.derivedLimits.maxTotalExposureUsd,
      tierNotionalCapUsd: profile.derivedLimits.maxMarginPerTradeUsd, defensiveMode: risk.sizeFactor < 1,
      liveMode: false, canaryActive: false, riskBudgetPct: profile.derivedLimits.maxRiskPerTradePct,
    ...
    if (d.rows.some(row => row.openDecisionId === id)) continue;
    if (!d.shouldContinue() || !await d.claim(id, { mode: 'VIRTUAL_PAPER_400', signal, decision, sizing,
      cost, sessionId: session.state.session.sessionId })) continue;
