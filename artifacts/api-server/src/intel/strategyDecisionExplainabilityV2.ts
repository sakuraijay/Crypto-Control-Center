...
function output(
  ...
  const { shadowRecord: shadow, riskDecision: risk, sizingAdvisory: sizing,
    confidenceAdvisory: confidence, gmxNetEdgeAdvisory: gmx } = input;
  return {
...
export function buildStrategyDecisionExplainabilityEnvelope(
  ...
  if (risk.action === 'REJECT') {
    if (sizing !== null || confidence !== null || gmx !== null) {
      return output(input, 'BLOCKED', 'INVALID', 0,
  ...
  if (sizing === null) {
    if (confidence !== null || gmx !== null) return output(input, 'BLOCKED', 'INVALID', 0,
  ...
  if (confidence === null) {
    if (gmx !== null) return output(input, 'BLOCKED', 'INVALID', 0,
  ...
  if (gmx === null) return output(input, 'NOT_EVALUATED', STRATEGY_DECISION_EXPLAINABILITY_VERSION, 0,
    'GMX Net Edge evidence 없음', ['비용을 0으로 위장하지 않음']);
  if (gmx.status === 'NOT_EVALUATED') return output(input, 'NOT_EVALUATED',
    STRATEGY_DECISION_EXPLAINABILITY_VERSION, 0,
    'GMX Context 또는 gross edge 미평가', [...gmx.reasons]);
  if (gmx.status === 'REJECTED') return output(input, 'REJECTED',
    STRATEGY_DECISION_EXPLAINABILITY_VERSION, 0,
