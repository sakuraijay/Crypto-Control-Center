...
## 확정 마일스톤 및 우선순위 — 2026-09-13 사용자 시각 변경

- **ALPHA: 2026-09-15 13:00 Asia/Manila (05:00 UTC).** 사용자의 최신 지시 “반드시 15일 오후 1시부터 알파 테스트 가능하도록 진행할것”에 따라 기존 정오를 오후 1시로 변경한다. 날짜·실제 400 USDC·무인 자동매매 범위는 유지한다.
...
- 사전 개발 마감은 유지: **9월 12일 18:00 필수 코드/검증 → 9월 13일 18:00 미완료 연결/검증 및 후보 배포/브라우저 확인 → 9월 14일 18:00 리허설/변경 동결 → 9월 15일 11:00~12:30 최종 점검/사용자 단계 → 12:30 Go/No-Go → 13:00 알파**. 이미 지난 마감의 완료 증거가 없으면 미완료/미확인으로 보고하며 소급 PASS 처리하지 않는다.
...
과거의 9월 15일 베타/정오 알파/10월 1일 정식 출시, 첫 LIVE 테스트의 매 주문 수동 승인 설명은 최신 알파·베타 일정 및 무인운용 합격 기준과 구분한다. 과거 파일을 다시 읽었다는 이유로 최신 일정을 되돌리거나 개별 주문 수동 시연을 무인 알파 완료로 보고하지 않는다. 이 문서 업데이트는 다른 대화/자동화가 실제로 읽거나 실행했다는 증거가 아니다.
