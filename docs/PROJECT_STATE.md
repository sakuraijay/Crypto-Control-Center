...
손실을 감수한 일일 적극적 PAPER 기록을 우선한다. 전용 실험 정책으로 최신 완료봉 모멘텀에 따라 매시간 진입을 시도하고 단타30분/스윙4시간 내 보호·청산을 수행한다. 거래를 검증된 일반전략 성과와 구분한다. [정책·검증·한계](verification/paper-daily-experiment/RESULT.md). CI/배포/운영 첫 진입은 이 커밋 시점 pending이다. 주간한도 절약을 위해 반복조회/중복작업을 줄인다.

## 이전 실행 지시 및 수정 — v13

문제 발견 시 감독이 해결 대안을 선택해 즉시 개발·검증·승인 범위 배포를 수행하고 실제 결과를 보고한다. 위험 문구만 반복하지 않는다. Virtual400 전략 가격 기반 신규 계획과 영속 무거래 진단을 구현하고 로컬 검증했다. [정확한 변경·과거 자료 비교·한계·배포 전 상태](verification/virtual400-structural-plan/RESULT.md). 이 커밋 시점 CI/게시 pending이며 자연 매매·일수익 목표 달성 증거가 아니다. 다음은 exact-source CI와 기존 세션을 보존한 PAPER 배포·새 진단 검증이다.

## 최신 핵심 목표 정정 — 2026-09-21 UTC

**CCC-SUPERVISION-20260921-12.** 매일 트레이딩과 계좌 하루 순수익5~10%가 사용자 제품목표다. 기존 포지션ROE 해석을 정정한다. 목표보장이나 달성확인이 아니다. [실제 무거래·정책불일치·개선수용기준](CCC_DAILY_TRADING_OBJECTIVE.md). 17:06UTC 운영확인:약26시간45분 OPEN/CLOSE/정산0,400유지,최근BTC/SOL고변동성전략비활성/UNI적격없음. 24h단계별집계자료미보존. P0영속진단→실데이터전략검증/개선→자연PAPER순정산. 이번변경은요구정정/점검이며제품기능배포아님.

## 이전 종목 선정 지시 — 2026-09-21

**CCC-SUPERVISION-20260921-11.** 고정 세 종목 대신 공식 GMX Arbitrum 전체 목록→검증된 적격 시장→영속 순환 batch→기존 전략/Risk/PAPER 실행 경로로 연결한다.
[구현 범위·검증·제한](CCC_MARKET_UNIVERSE.md). 시장 목록 조회와 실제 자연 매매 성공을 혼동하지 않는다. 비용 데이터 부족은 독립 P0이며 임의로0대체하지 않는다.
